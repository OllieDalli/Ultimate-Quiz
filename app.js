...
// Timing diagnostics. These are intentionally lightweight and are used to
// identify whether a timing problem is coming from the device clock, browser
// timer throttling, Realtime delivery, or network latency.
const timingDiagnostics = {
    clockOffsetMs: null,
    realtimeStatus: "NOT STARTED",
    lastRealtimeEventAt: null,
    lastQuestionKey: null,
    questionRenderAt: null,
    sharedStartMs: null,
    lastAnswerClickAt: null,
    lastAnswerElapsedMs: null,
    maxTimerDriftMs: null,
    visibilityChanges: 0,
...
function setupTimingDiagnostics() {
    const button = document.getElementById("timing-diagnostics-button");
    const closeButton = document.getElementById("timing-diagnostics-close");
    const runButton = document.getElementById("timing-diagnostics-run");
    const modal = document.getElementById("timing-diagnostics-modal");

    if (!button || !modal) return;

    button.addEventListener("click", () => {
        modal.classList.remove("hidden");
        renderTimingDiagnostics({ running: false });
        runTimingDiagnostics();
    });

    closeButton?.addEventListener("click", () => modal.classList.add("hidden"));
    runButton?.addEventListener("click", () => runTimingDiagnostics());
    ...
    document.addEventListener("visibilitychange", () => {
        timingDiagnostics.visibilityChanges += 1;
        updateTimingDiagnosticBadge();
    });
}
...
function timingStatus(value, good, warn) {
    if (good) return `<span class="timing-ok">OK</span>`;
    if (warn) return `<span class="timing-warn">CHECK</span>`;
    return `<span class="timing-bad">ISSUE</span>`;
}

function updateTimingDiagnosticBadge() {
    const badge = document.getElementById("timing-diagnostics-badge");
    if (!badge) return;

    const offset = timingDiagnostics.clockOffsetMs;
    const drift = timingDiagnostics.maxTimerDriftMs;
    const realtime = timingDiagnostics.realtimeStatus;
    const hasIssue = (Number.isFinite(offset) && Math.abs(offset) > 1000) ||
        (Number.isFinite(drift) && drift > 250) ||
        (realtime && !["SUBSCRIBED", "CHANNEL_ERROR"].includes(realtime));

    badge.textContent = hasIssue ? "Timing check: CHECK" : "Timing check";
    badge.classList.toggle("timing-badge-warning", hasIssue);
}

async function measureTimerDrift() {
    const samples = [];
...
async function runTimingDiagnostics() {
    if (timingDiagnostics.diagnosticRunning) return;
    timingDiagnostics.diagnosticRunning = true;
    renderTimingDiagnostics({ running: true });
    ...
    if (game?.id) {
        ...
        const { data, error } = await supabaseClient
            .from("games")
            .select("id,created_at,phase,round,current_round_type,current_question_id,question_started_at")
            .eq("id", game.id)
            .single();
        pingMs = performance.now() - pingStart;
        serverGame = data || null;
        dbError = error || null;
    }

    if (!Number.isFinite(timingDiagnostics.clockOffsetMs) && serverGame?.created_at) {
        // This is only a rough diagnostic after reconnect. A fresh create/join
        // measurement is more accurate because the elapsed game age is known.
        timingDiagnostics.clockOffsetMs = null;
    }

    try {
        timingDiagnostics.maxTimerDriftMs = await measureTimerDrift();
    } catch (error) {
        console.warn("Timing drift diagnostic failed:", error);
        timingDiagnostics.maxTimerDriftMs = null;
    }

    const elapsed = performance.now() - started;
    const offset = timingDiagnostics.clockOffsetMs;
    const sharedStart = serverGame?.question_started_at
        ? new Date(serverGame.question_started_at).getTime()
        : timingDiagnostics.sharedStartMs;
    const sharedDelta = Number.isFinite(sharedStart)
        ? Date.now() - sharedStart
        : null;
    const browser = navigator.userAgentData?.brands?.map(item => item.brand).join(", ") || navigator.userAgent;
    const connection = navigator.connection || navigator.mozConnection || navigator.webkitConnection;
    const network = connection
        ? `${connection.effectiveType || "unknown"}${Number.isFinite(connection.rtt) ? `, browser RTT ${connection.rtt} ms` : ""}`
        : "Not exposed by this browser";

    let diagnosis = "No obvious local timing problem detected.";
    let detail = "If the problem happens again, run this check while the broken timing is visible and send the result to the host/developer.";

    if (Number.isFinite(offset) && Math.abs(offset) > 2000) {
        diagnosis = "LIKELY DEVICE CLOCK PROBLEM";
        detail = isHost
            ? "This device is using a clock more than 2 seconds away from the database clock. Because you are the host, your shared question start times can make timing wrong for everyone. Turn on Windows automatic time synchronisation and test again."
            : "This device's clock differs significantly from the measured database clock. That can make local timing checks and answer windows behave incorrectly. Turn on automatic time synchronisation and test again.";
    } else if (Number.isFinite(timingDiagnostics.maxTimerDriftMs) && timingDiagnostics.maxTimerDriftMs > 250) {
        diagnosis = "LIKELY BROWSER/TIMER DELAY";
        detail = "The browser delayed a timer by more than 250 ms during the diagnostic. Background tabs, extensions, power-saving mode, high CPU usage, or browser performance issues can cause this."
    } else if (timingDiagnostics.realtimeStatus !== "SUBSCRIBED") {
        diagnosis = "REALTIME CONNECTION NEEDS CHECKING";
        detail = `The quiz's live Supabase connection currently reports ${timingDiagnostics.realtimeStatus}. Realtime delays can stop question changes or answer updates arriving promptly.`;
    } else if (Number.isFinite(pingMs) && pingMs > 800) {
        diagnosis = "NETWORK LATENCY IS HIGH";
        detail = "The database round trip was over 800 ms. That can make shared state feel delayed, although the quiz should still use the shared timestamp for the 10-second window.";
    } else if (Number.isFinite(sharedDelta) && sharedDelta < -2000) {
        diagnosis = "SHARED START TIME IS IN THE FUTURE";
        detail = "This device received a question whose shared start time is more than 2 seconds in the future. If this happens repeatedly, check the host's device clock first."
    } else if (Number.isFinite(sharedDelta) && sharedDelta > 12000) {
        diagnosis = "QUESTION START TIME LOOKS STALE";
        detail = "The current shared question timestamp is more than 12 seconds old. If this appears during a live question, the game state or Realtime connection may be stale."
    }

    timingDiagnostics.diagnosticRunning = false;
...
function renderTimingDiagnostics({ running = false, diagnosis = "Running checks…", detail = "", pingMs = null, sharedDelta = null, browser = "", network = "", dbError = null, elapsed = null } = {}) {
    const output = document.getElementById("timing-diagnostics-output");
    if (!output) return;
    ...
    if (running) {
        output.innerHTML = `<div class="timing-diagnosis running">Running device, browser, network and live-quiz timing checks…</div>`;
        return;
    }

    const clockGood = Number.isFinite(offset) ? Math.abs(offset) <= 1000 : false;
    const driftGood = Number.isFinite(drift) ? drift <= 150 : false;
    const realtimeGood = realtime === "SUBSCRIBED";
    const pingGood = Number.isFinite(pingMs) ? pingMs <= 300 : false;

    output.innerHTML = `
        <div class="timing-diagnosis ${diagnosis.includes("LIKELY") || diagnosis.includes("NEEDS") || diagnosis.includes("HIGH") || diagnosis.includes("STALE") || diagnosis.includes("FUTURE") ? "problem" : "good"}">
            <strong>${diagnosis}</strong>
            <span>${detail}</span>
        </div>
        <div class="timing-grid">
            <div><b>Device clock</b><span>${Number.isFinite(offset) ? formatTimingMs(offset) : "Not measured yet"} ${Number.isFinite(offset) ? timingStatus(offset, clockGood, Math.abs(offset) <= 2000) : ""}</span></div>
            <div><b>Browser timer drift</b><span>${Number.isFinite(drift) ? `up to ${Math.round(drift)} ms` : "Not measured"} ${Number.isFinite(drift) ? timingStatus(drift, driftGood, drift <= 250) : ""}</span></div>
            <div><b>Supabase round trip</b><span>${Number.isFinite(pingMs) ? `${Math.round(pingMs)} ms` : "Not measured"} ${Number.isFinite(pingMs) ? timingStatus(pingMs, pingGood, pingMs <= 800) : ""}</span></div>
            <div><b>Realtime</b><span>${realtime} ${timingStatus(realtime, realtimeGood, realtime === "CHANNEL_ERROR")}</span></div>
            <div><b>Shared question clock</b><span>${Number.isFinite(sharedDelta) ? `${formatTimingMs(sharedDelta)} from start` : "No live question"}</span></div>
            <div><b>Visibility changes</b><span>${timingDiagnostics.visibilityChanges}</span></div>
            <div><b>Role</b><span>${isHost ? "Host" : "Player"}</span></div>
            <div><b>Last answer timing</b><span>${Number.isFinite(timingDiagnostics.lastAnswerElapsedMs) ? `${(timingDiagnostics.lastAnswerElapsedMs / 1000).toFixed(3)} s` : "No answer captured"}</span></div>
        </div>
        <div class="timing-meta">
            <div><b>Browser:</b> ${escapeHtml(browser || navigator.userAgent)}</div>
            <div><b>Network:</b> ${escapeHtml(network || "Unknown")}</div>
            ${dbError ? `<div class="timing-error"><b>Database check:</b> ${escapeHtml(dbError.message || "Request failed")}</div>` : ""}
            ${Number.isFinite(elapsed) ? `<div><b>Diagnostic duration:</b> ${Math.round(elapsed)} ms</div>` : ""}
        </div>`;
}
...
async function createGame() {
    ...
    // The returned created_at is generated by Postgres. Comparing it with the
    // local clock immediately after creation gives us a useful device-clock
    // offset without needing a new database function.
    if (newGame?.created_at) {
        timingDiagnostics.clockOffsetMs = Date.now() - new Date(newGame.created_at).getTime();
    }
...
async function joinGame() {
    ...
    const { data: joinedPlayer, error: playerError } =
        await supabaseClient
            .from("players")
            .insert({
                id: playerId,
    ...
    if (joinedPlayer?.joined_at) {
        timingDiagnostics.clockOffsetMs = Date.now() - new Date(joinedPlayer.joined_at).getTime();
    }
        ...
        supabaseClient
            ...
            .on(
                ...
                payload => {
                    ...
                    timingDiagnostics.lastRealtimeEventAt = Date.now();
            ...
            .subscribe(status => {
                timingDiagnostics.realtimeStatus = status || "UNKNOWN";
                updateTimingDiagnosticBadge();
            });
...
function handleRoundIntroPhase() {
    ...
    phase.innerHTML = `
        <div class="round-intro-card">
            ...
            <p class="round-intro-round">Round ${round} of 10</p>
...
function updateGameState() {
    ...
    if (game.phase === "question" && ["song", "speed"].includes(game.current_round_type)) {
        const questionKey = `${game.round}:${game.current_round_type}:${game.current_question_id}:${game.question_started_at || ""}`;
        if (timingDiagnostics.lastQuestionKey !== questionKey) {
            timingDiagnostics.lastQuestionKey = questionKey;
            timingDiagnostics.sharedStartMs = new Date(game.question_started_at || 0).getTime();
            timingDiagnostics.questionRenderAt = Date.now();
            timingDiagnostics.lastAnswerClickAt = null;
            timingDiagnostics.lastAnswerElapsedMs = null;
            timingDiagnostics.maxTimerDriftMs = null;
        }
    }
...
async function submitAnswer(answer) {
    ...
    if (isSongRound || isSpeedRound) {
        ...
        if (answer !== "__TIMEOUT__") {
            if (isSpeedRound) {
                // Speed uses the shared question_started_at clock directly.
                // This avoids local setTimeout/performance anchors drifting when
                // a browser is busy or several players answer at once.
                responseElapsedMs = Math.max(0, Math.min(10000, now - timing.startMs));
            } else {
                const startPerf = songLocalStartPerf;
                if (startPerf !== null && typeof performance !== "undefined") {
                    responseElapsedMs = Math.max(0, Math.min(10000, performance.now() - startPerf));
                } else {
                    responseElapsedMs = Math.max(0, Math.min(10000, now - timing.startMs));
                }
            }
    ...
    // Record local timing diagnostics before any database operation.
    if (isSongRound || isSpeedRound) {
        timingDiagnostics.lastAnswerClickAt = Date.now();
        timingDiagnostics.lastAnswerElapsedMs = responseElapsedMs;
    }
...
async function renderSpeedResponseTimes() {
    ...
    const rows = players.map(player => {
        ...
        return {
            ...
            points: Number(answerRow?.points_awarded || 0),
...
function createRandomRoundTypeOrder(totalRounds = 10) {
    // Keep the game varied while guaranteeing one Winner Takes All round:
    // 3 Classic, 3 Name That Song, 3 Speed and 1 Winner Takes All.
    const counts = { classic: 3, song: 3, speed: 3, wta: 1 };
    const order = [];
...
async function handleLeaderboardPhase() {
    ...
    const isFinalRound = Number(game.round || 1) >= 10;
    const bonusIsDue =
        !isFinalRound &&
        !game.bonus_used &&
...
async function continueFromLeaderboard() {
    ...
    try {
        ...
        if (continueButton && game?.phase === "leaderboard") {
            ...
            const finalRound = Number(game?.round || 1) >= 10;
...
async function startNextRound() {
    ...
    // The quiz is exactly 10 rounds. After round 10's leaderboard, move
    // directly to the final results instead of creating an 11th round.
    if (Number(game.round || 1) >= 10) {
        const { error } = await supabaseClient
            .from("games")
            .update({
                phase: "final",
    ...
    game = { ...game, round: nextRound, current_round_type: nextRoundType, phase: "round-intro", current_question_id: null, current_answer: null, question_started_at: null, wta_awarded: false };
    updateGameState();
}
