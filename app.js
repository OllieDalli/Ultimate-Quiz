...
async function beginRound() {
    ...
    game = { ...game, current_round_type: roundType, phase: "round-intro", current_question_id: null, current_answer: null, question_started_at: null, wta_awarded: false };
    updateGameState();
}
...
async function submitAnswer(answer) {
    ...
    if (isSongRound || isSpeedRound) {
        ...
        if (answer !== "__TIMEOUT__") {
            const startPerf = isSongRound ? songLocalStartPerf : speedLocalStartPerf;
            if (startPerf !== null && typeof performance !== "undefined") {
                responseElapsedMs = Math.max(0, Math.min(10000, performance.now() - startPerf));
            } else {
                // Fallback if the browser missed the local start callback.
                // The shared start timestamp is still authoritative.
...
async function startSpeedRound() {
    ...
    if (error) {
        console.error("Could not check used speed questions:", error);
        return;
    }
    ...
    const speedQuestion = getRandomSpeedQuestion(usedQuestionIds);
    if (!speedQuestion) {
...
function startSpeedTimer() {
    ...
    const tick = () => {
        ...
        if (!beforeStart && speedLocalStartPerf === null) {
            speedLocalStartPerf = performance.now();
        }
    ...
    speedPreStartTimeout = setTimeout(() => {
        if (speedLocalStartPerf === null) speedLocalStartPerf = performance.now();
        updateSpeedAnswerControls();
    }, delay);
...
async function renderSpeedResponseTimes() {
    ...
    const { data: answerRows, error } = await supabaseClient
        .from("answers")
        .select("player_id,answered_at,correct,points_awarded,answer,difficulty")
        .eq("game_id", game.id)
        .eq("round", game.round);
    ...
    const startMs = new Date(game.question_started_at || 0).getTime();
    const roundDifficulty = game.current_round_type === "song" ? "song" : "speed";
    const relevantAnswers = (answerRows || []).filter(row => row.difficulty === roundDifficulty);

    // Calculate the displayed points directly from answer order as well as
    // reading them from the database. This keeps the response-time panel
    // correct even if the reveal arrives before the points_awarded updates
    // have propagated to every client.
    const correctInOrder = relevantAnswers
        .filter(row => row.correct && row.answered_at)
        .sort((a, b) => new Date(a.answered_at).getTime() - new Date(b.answered_at).getTime());
    const calculatedPoints = new Map();
    correctInOrder.forEach((row, index) => {
        calculatedPoints.set(row.player_id, index === 0 ? 6 : index === 1 ? 4 : index === 2 ? 2 : 1);
    });

    const rows = players.map(player => {
        const answerRow = relevantAnswers.find(row => row.player_id === player.id);
        let elapsedMs = null;
        ...
        return {
            ...
            points: calculatedPoints.get(player.id) || 0,
