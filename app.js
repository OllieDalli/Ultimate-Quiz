...
// ============================================================
// CLASSIC QUESTION RENDERING / TIMER
// ============================================================
...
function renderQuestion() {
    ...
    if (banner) {
        const type = currentRoundType === "song" ? "song" : "classic";
    ...
    if (songPlayButton) {
        songPlayButton.classList.toggle("hidden", currentRoundType !== "song");
        songPlayButton.textContent = "▶ Play Clip";
    }
...
function updateAnswerButtons() {
    const isSongRound = (game?.current_round_type || currentRoundType) === "song";

    const canAnswer = isSongRound
        ? game?.phase === "question" && !answerSubmitted
        : game?.phase === "question" &&
          game?.current_player_id === myPlayerId &&
          !answerSubmitted;
...
function startTimer() {
    ...
    const tick = () => {
        if (!game || game.phase !== "question" ||
            (game.current_round_type || currentRoundType) === "song") {
            clearInterval(timerInterval);
            timerInterval = null;
            return;
        }

        const remaining = Math.max(0, Math.ceil((startMs + durationMs - Date.now()) / 1000));
        ...
        if (remaining <= 0) {
            ...
            if (game.current_player_id === myPlayerId && !answerSubmitted) {
...
async function submitAnswer(answer) {
    if (answerSubmitted || !game || !selectedQuestion) return;

    const isSongRound = (game.current_round_type || currentRoundType) === "song";

    if (game.phase !== "question") return;

    if (!isSongRound && game.current_player_id !== myPlayerId) return;

    // Lock immediately. This prevents double-clicks and repeated realtime
    // events from awarding the same answer twice.
    answerSubmitted = true;

    // For Classic rounds we deliberately keep the timer running while the
    // short answer-feedback animation is shown. Stopping the interval here
    // made the countdown appear to freeze and could leave the game looking
    // stuck if the transition was interrupted. Song rounds manage their own
    // shared timer and should stop it when the answering window ends.
    if (isSongRound) {
        clearInterval(timerInterval);
        timerInterval = null;
    }
    ...
    const correct = !isTimeout && answer === selectedQuestion.correct;
    const points = correct
        ? (isSongRound ? 3 : Number(POINTS[game.current_difficulty] || 0))
        : 0;

    // Show the selection locally first. Do not write a player's live
    // selection into shared game state during song rounds.
    if (!isTimeout) {
        const selectedButton = document.querySelector(
            `.answer-button[data-answer="${CSS.escape(answer)}"]`
        );
        if (selectedButton) {
            selectedButton.classList.add("answer-selected-amber");
        }
    }
    ...
    if (answerError) {
        console.error("Could not save answer:", answerError);
        answerSubmitted = false;
        updateAnswerButtons();
        return;
    }

    // Classic rounds may share the selected answer after it has been locked.
    // This is intentionally never used for live music-round selections.
    if (!isSongRound) {
        const { error: sharedAnswerError } = await supabaseClient
            .from("games")
            .update({ current_answer: isTimeout ? null : answer })
            .eq("id", game.id);

        if (sharedAnswerError) {
            console.error("Could not save classic answer feedback:", sharedAnswerError);
        }
    }
    ...
    if (points > 0) {
        ...
        if (player) {
            const newPoints = Number(player.points || 0) + points;
            ...
            if (scoreError) {
                console.error("Could not update score:", scoreError);
            }
        }
    }
    ...
    // Give everyone a brief moment to see the classic answer feedback,
    // then return control to the host to select the next player. Do not rely
    // on the local isHost flag here: it can briefly be stale after a realtime
    // game update. The database game row is authoritative.
    if (!isSongRound) {
        clearTimeout(nextPlayerTimeout);
        nextPlayerTimeout = setTimeout(async () => {
            if (!game || game.phase !== "question") return;

            const localIsHost = game.host_id === myPlayerId;
            ...
            const { data: updatedGame, error: phaseError } =
                await supabaseClient
                    .from("games")
                    .update(nextState)
                    .eq("id", game.id)
            ...
            if (phaseError) {
                console.error("Could not start next player selection:", phaseError);
                // Retry once shortly afterwards in case a transient realtime
                // update raced with this transition.
                nextPlayerTimeout = setTimeout(async () => {
                    if (game?.phase === "question" && game.host_id === myPlayerId) {
                        const { error: retryError } = await supabaseClient
                            ...
                            .eq("phase", "question");
                        if (retryError) {
                            console.error("Retry could not start next player selection:", retryError);
                        }
                    }
                }, 500);
                return;
