...
function setupGameButtons() {
    ...
    // End Game
    const endGameButton =
        document.getElementById("end-game-button") ||
        document.getElementById("new-round-button");

    if (endGameButton && endGameButton.dataset.bound !== "true") {
        endGameButton.dataset.bound = "true";
...
async function endGame() {
    ...
    const button = document.getElementById("end-game-button") || document.getElementById("new-round-button");
    if (button) {
        button.disabled = true;
        button.textContent = "Ending Game…";
    }
    ...
    returnToHomeAfterGameEnd();
}

function returnToHomeAfterGameEnd() {
    clearTimers();
    ...
    const { data: newGame, error: gameError } =
        await supabaseClient
            ...
            .insert({
                ...
                bonus_round: Math.floor(Math.random() * 9) + 1,
                bonus_used: false,
...
function updateGameState() {
    ...
    // Hard stop for any legacy game that was created with the old 12-round
    // configuration. Round 10 is now the maximum. The host moves the shared
    // game state to the final screen so every connected player follows it.
    if (Number(game.round || 1) > 10 && game.phase !== "final" && game.phase !== "ended") {
        if (game.host_id === myPlayerId) {
            supabaseClient
                .from("games")
                .update({
                    phase: "final",
                ...
                .eq("id", game.id)
                .then(({ error }) => {
                    if (error) console.error("Could not stop legacy game at round 10:", error);
                });
        }
...
function updateAnswerButtons() {
    ...
    const canAnswer = isSongRound
        ? !answerSubmitted && game.phase === "question"
        : game.current_player_id === myPlayerId && !answerSubmitted;
...
async function submitAnswer(answer) {
    ...
    if (!isSongRound && game.current_player_id !== myPlayerId) {
        return;
    }
    ...
    const points = correct
        ? (isSongRound ? 3 : POINTS[game.current_difficulty])
        : 0;
    ...
    nextPlayerTimeout = setTimeout(async () => {
        const isFinalRound = Number(game.round) >= 10;
        const { error: phaseError } = await supabaseClient
            .from("games")
            .update({
                phase: isFinalRound ? "final" : "selecting",
                current_player_id: null,
                current_difficulty: null,
                current_question_id: null,
                current_answer: null,
...
function createRandomRoundTypeOrder(totalRounds = 10) {
    ...
    // Build an even 5/5 pool, then fully Fisher-Yates shuffle it.
    // Consecutive identical types are deliberately allowed.
    for (let i = 0; i < classicCount; i++) order.push("classic");
    for (let i = 0; i < songCount; i++) order.push("song");
...
function handleSongRevealPhase() {
    ...
    if (isHost) {
        songRevealTimeout = setTimeout(async () => {
            ...
            const nextPhase = Number(game.round) >= 10 ? "final" : "leaderboard";
            await supabaseClient
                .from("games")
                .update({
                    phase: nextPhase,
...
function renderLeaderboard() {
    ...
    if (endGameButton) {
        endGameButton.classList.toggle("hidden", !isHost);
        endGameButton.style.display = isHost ? "block" : "none";
    }
...
async function startNextRound() {
    ...
    if (!isHost) return;
    if (!game || Number(game.round || 1) >= 10) return;
