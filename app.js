...
async function startSongRound() {
    ...
    const songQuestion = getRandomSongQuestion(usedQuestionIds);
    if (!songQuestion) {
...
function handleSongQuestionPhase() {
    const questionId = game.current_question_id;
    const songQuestion = getSongQuestionById(questionId);
    if (!songQuestion) return;
...
function startSongTimer() {
    ...
    const tick = () => {
        ...
        const remaining = Math.max(0, Math.ceil((answerWindowMs - elapsed) / 1000));
...
async function finishSongRoundIfNeeded() {
    ...
    if ((latestAnswers || []).length >= players.length) {
...
async function finishSongRoundIfEveryoneAnswered() {
    if (!game || game.phase !== "question") return;
    ...
    if ((latestAnswers || []).length < players.length) return;
    if (!isHost) return;
...
async function handleLeaderboardPhase() {
    ...
    if (!continueButton || !game) return;

    // IMPORTANT: the leaderboard is now a hard pause after every round.
    // Nothing advances automatically. Only the host can move the game on.
    if (isHost) {
        ...
        continueButton.textContent = bonusIsDue
