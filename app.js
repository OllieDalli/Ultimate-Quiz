...
function setupGameButtons() {
    ...
    // End game from final leaderboard
    const endGameButton =
            ...
            if (!availablePlayers.length) {
                ...
                const { error: phaseError } =
                    await supabaseClient
                        .from("games")
                        .update({
                            phase: isFinalRound ? "final" : "leaderboard",
                            current_player_id: null,
                            current_difficulty: null,
                            current_question_id: null,
                            question_started_at: null,
                ...
                if (phaseError) {
                    console.error("Could not finish the round:", phaseError);
                }
...
function createRandomRoundTypeOrder(totalRounds = 10) {
    // Five Classic and five Name That Song rounds, but with a completely
    // random order. Consecutive rounds of the same type are allowed.
    const order = [];
    const classicCount = Math.floor(totalRounds / 2);
    const songCount = totalRounds - classicCount;
    ...
    if (isHost) {
        songRevealTimeout = setTimeout(async () => {
            ...
            await supabaseClient
                .from("games")
                .update({
                    phase: Number(game.round || 1) >= 10 ? "final" : "leaderboard",
                    current_answer: null,
                    current_question_id: null,
                    question_started_at: null
                })
...
function renderFinalLeaderboard() {
    ...
    if (endButton) {
        ...
        endButton.textContent = isHost ? "End Game" : "Waiting for Host";
    }
...
async function endGame() {
    if (!game || !isHost || game.phase !== "final") return;
