...
function markLocalSpeedSelection(answer) {
    ...
    const isReveal = game?.phase === "speed-reveal";

    document.querySelectorAll(".answer-button").forEach(button => {
        const isSelected = button.dataset.answer === answer;

        // During the live answering window, show the player's selection in
        // amber. During the reveal, do NOT re-apply the amber class because
        // it overrides the green correct-answer state (and red wrong-answer
        // state) with !important CSS.
        if (!isReveal) {
            button.classList.toggle("speed-local-selected", isSelected);
        } else {
            button.classList.remove("speed-local-selected");
        }
