...
async function beginRound() {
    ...
    await supabaseClient
        .from("games")
        .update({
            phase: (roundType === "song" || roundType === "speed") ? "question" : "selecting",
            current_player_id: null,
            current_difficulty: null,
            current_question_id: null,
            current_answer: null,
    ...
    game = { ...game, current_round_type: roundType, phase: (roundType === "song" || roundType === "speed") ? "question" : "selecting", current_question_id: null, current_answer: null, question_started_at: null };
...
function markLocalSongSelection(answer) {
    ...
    // Show the player's selection in amber immediately. The answer stays
    // amber until the shared song round reaches the reveal phase.
    // The YOUR PICK badge remains visible throughout.
...
function renderQuestion() {
    ...
    // Shuffle the answer positions for every song question render.
    // Classic questions keep their existing order.
    const answersToRender = (currentRoundType === "song" || currentRoundType === "speed")
...
async function submitAnswer(answer) {
    ...
    // Do not stop the shared song/speed timer when a player answers.
    // The round must continue running until its shared deadline or until
    ...
    if (correct) {
        if (isSongRound) {
            ...
            const elapsedMs = Math.max(0, Date.now() - getSpeedTiming().startMs);
            points = getSpeedPoints(elapsedMs);
        } else {
    ...
    if (isSongRound) {
        await finishSongRoundIfEveryoneAnswered();
        return;
    }
...
function startSpeedTimer() {
    ...
    speedPreStartTimeout = setTimeout(updateSpeedAnswerControls, delay);
...
async function finishSpeedRoundIfNeeded() {
    if (!isHost || speedRoundFinished || speedDeadlineHandled || !game) return;
    if (game.phase !== "question" || game.current_round_type !== "speed") return;
...
function createRandomRoundTypeOrder(totalRounds = 10) {
    // Keep the game varied while guaranteeing a meaningful mix:
    // 4 Classic, 3 Name That Song and 3 Speed rounds in a 10-round game.
    const counts = { classic: 4, song: 3, speed: 3 };
    const order = [];
...
function renderLeaderboard() {
    ...
    // loadPlayers() also calls renderLeaderboard(), including while the game
    // is in question/difficulty/selecting phases. Never consume the current
    // round here, otherwise the real leaderboard render would think the
    // animation has already happened. The animation belongs ONLY to the
    // moment the shared game phase becomes leaderboard.
...
async function startNextRound() {
    ...
    await supabaseClient
        ...
        .update({
            round: nextRound,
            phase: (nextRoundType === "song" || nextRoundType === "speed") ? "question" : "selecting",
            current_player_id: null,
            current_difficulty: null,
            current_question_id: null,
            current_answer: null,
    ...
    game = { ...game, round: nextRound, current_round_type: nextRoundType, phase: (nextRoundType === "song" || nextRoundType === "speed") ? "question" : "selecting", current_question_id: null, current_answer: null, question_started_at: null };

    if (nextRoundType === "song") {
        await startSongRound();
    } else if (nextRoundType === "speed") {
        await startSpeedRound();
    } else {
        setTimeout(async () => {
            await selectNextPlayer();
